...
def render_broadsheet_markdown(config: MorningPaperConfig, collected: dict[str, list[SourceItem]], *, date_str: str) -> str:
    ...
    replacements = {
        ...
        "<!-- Reference links -->": (
            '<div class="dept-list">'
            "<p><strong>Sources:</strong> configured feeds, staged material, and local collectors</p>"
...
def _staged_section(config: MorningPaperConfig, date_str: str) -> tuple[str, list[str], list[str]]:
    ...
    consume what `stage` queued — staged items silently vanishing from the
    edition was the 0.4.3 P0. Warnings are loud and specific: a queue that
    ...
    try:
        ...
        return "", [], [
            f"STAGED ITEMS NOT INCLUDED: staging queue exists for {date_str} but could not be read "
            f"({queue_file}): {exc}"
        ]
    ...
    for item in items:
        ...
        if not slug or not md_path.is_file():
            warnings.append(
                f"STAGED ITEM NOT INCLUDED: '{item.get('title') or slug or '?'}' — staged file missing: {md_path}"
            )
            continue
        try:
            ...
            warnings.append(
                f"STAGED ITEM NOT INCLUDED: '{item.get('title') or slug}' — staged file unreadable: {exc}"
            )
            continue
...
def write_outputs(
    ...
    if config.outputs.renderer == "typewriter":
        ...
        if would_include:
            warnings.append(
                f"STAGED ITEMS NOT INCLUDED: {len(would_include)} item(s) are queued for {date_str} but the "
                "portable renderer cannot typeset staged markdown; set `outputs.renderer: typewriter` "
                "to include them"
            )
    payload = {
