...
class SourcesCliTest(unittest.TestCase):
    ...
    def test_sources_list_can_include_newsroom_collectors(self) -> None:
        ...
        self.assertEqual(payload["configured_collectors"]["count"], len(payload["newsroom"]["collectors"]))
        self.assertIn("collector:local-drop", payload["configured_collectors"]["ids"])
        self.assertEqual(payload["editorial_source_ledger"]["status"], "present")
        self.assertFalse(payload["editorial_source_ledger"]["executable"])
        self.assertGreaterEqual(payload["editorial_source_ledger"]["row_count"], 1)
        self.assertIn("not an executable source registry", payload["editorial_source_ledger"]["meaning"])
